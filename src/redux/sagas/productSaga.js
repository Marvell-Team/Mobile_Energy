...
import {getProduct, getProductByCategorys,getProductByCategorysChild,getProductById} from '../api/product'
import { GET_PRODUCT,GET_PRODUCT_SUCCESS,GET_PRODUCT_ERROR, GET_PRODUCT_BY_CATEGORYS, GET_PRODUCT_BY_CATEGORYS_SUCCESS, GET_PRODUCT_BY_CATEGORYS_ERROR, GET_PRODUCT_BY_ID, GET_PRODUCT_BY_ID_SUCCESS, GET_PRODUCT_BY_ID_ERROR, GET_PRODUCT_BY_CATEGORYS_CHILD, GET_PRODUCT_BY_CATEGORYS_CHILD_ERROR, GET_PRODUCT_BY_CATEGORYS_CHILD_SUCCESS } from "../actions/ProductAction";
...
function* getProductByCategoriesSaga(action){
    ...
    if(response !==undefined && response!==null){
        if(response.status===1){
            ...
            //truong hợp này gọi tới sever bi sever báo lỗi về
            yield put({type:GET_PRODUCT_BY_CATEGORYS_ERROR,error:response.error})
        }
     }else{
         //Đôi khi api lỗi , sever ko trả dữ liệu vè
         const message="Có lỗi xảy ra, không thể kết nối tới sever"
...
export function* watchgetProductByCategoriesChildSaga(){
    yield takeEvery(GET_PRODUCT_BY_CATEGORYS_CHILD,getProductByCategoriesChildSaga)
}
function* getProductByCategoriesChildSaga(action){
    ...
    if(response !==undefined && response!==null){ 
        if(response.status===1){
            ...
            //truong hợp này gọi tới sever bi sever báo lỗi về
            yield put({type:GET_PRODUCT_BY_CATEGORYS_CHILD_ERROR,error:response.error})
        }
     }else{
         //Đôi khi api lỗi , sever ko trả dữ liệu vè
         const message="Có lỗi xảy ra, không thể kết nối tới sever"
